...
namespace MyWeb_Hub.DAL
{
    public class ChatDataDLL
    {
        ...
        {
            string sql = " SELECT TOP 100 [id] ,[Sender] ,[Receiver] ,[Time],[Msg] FROM [Chat].[dbo].[chatdata] where (Sender='{0}' and Receiver='{1}') or(Receiver='{2}' and Sender='{3}') order by [Time]  desc";
            sql = string.Format(sql,sender,receiver, sender, receiver);
