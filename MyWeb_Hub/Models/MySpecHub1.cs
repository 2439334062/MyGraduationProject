...
namespace MyWeb_Hub.Models
{
    public class MySpecHub1 : Hub
    {
        ...
        public void SendConnIdMsg(string name,string msg)
        {
            ...
            string sendMsg = "[{\"uName\":\""+ SenderName + "\",\"msg\":\"" + msg+ "\"}]";
            Clients.Client(connid).receiveMsg($"{sendMsg}");
        }
        /// <summary>
        /// 点对点发送消息，获取数据库里面的connectionId来发送消息 
        ///
        ...
        public void EnterRoom(string roomName)
        {
            ...
            string sendMsg = "[{\"uName\":\"" + roomName + "\",\"sendName\":\"" + sendName + "\"}]";
            // Clients.Client(this.Context.ConnectionId).receiveroomMsg($"用户【{sendName}】成功进入房间：【{roomName}】");
            Clients.Group(roomName, this.Context.ConnectionId).receiveroomMsg($"{sendMsg}进入房间");
        }
        #endregion
        ...
        public void SendRoomNameMsg(string roomName, string msg)
        {
            ...
            //向指定组发送消息，如果这个组包含自己，将自己除外
            // Clients.Group(roomName, this.Context.ConnectionId).receiveMsg($"用户【{this.Context.ConnectionId}】发来消息：{msg}");
            string sendMsg = "[{\"uName\":\"" + roomName+ "\",\"msg\":\"" + msg+ "\"}]";
             Clients.Group(roomName, this.Context.ConnectionId).receiveMsg($"{sendMsg}");
        }
        ...
        public void AddGroupChat(string roomName)
        {
            ...
            if (a > 0)
            {
                if (a == 1)
                {
                    Clients.Client(this.Context.ConnectionId).roomadd("ok");
                }
                else if (a == 3)
                {
                    Clients.Client(this.Context.ConnectionId).bbb("已经加入该群");
                }
                else
                {
                    Clients.Client(this.Context.ConnectionId).roomadd("on");
        ...
        {
            DataTable dt = new GroupChatDataBLL().GetRoomDataBLL(roomName);
            int len = dt.Rows.Count;
            string[] strMsg = new string[len];
            Clients.Client(this.Context.ConnectionId).dataMsg("2121");
            //for (int j = 0; j <len; j++)
            //{
            //    Clients.Client(this.Context.ConnectionId).dataMsg($"{strMsg[j]}");//输出群
            //}
        }
